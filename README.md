...
EIT는 생체 전기 임피던스 데이터 \(\mathbf{V} \in \mathbb{R}^m\)로부터 인체 내 전기 전도도 분포 \(\mathbf{r} \in \mathbb{R}^n\)를 영상화하므로, \(\mathbf{V}\)에서 \(\mathbf{r}\)로 가는 함수 \(f\)를 찾는 문제로 볼 수 있습니다: \(f(\mathbf{V}) = \mathbf{r}\).
